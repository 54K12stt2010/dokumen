...
<div class="row">
    ...
    <?php
     $tampil = mysqlI_query($con,"SELECT * FROM standart where id_standart <> '4'    ");
     $no=1;
        ...
        <div class="x_panel">
            ...
            <div class="x_content">
                ...
                <?php 
                    ...
                    <input type="hidden" name="menu_isi" value="<?php echo"$_GET[id_menu]";?>"  >
                    <input type="hidden" name="id_standart" value="<?php echo"$r[id_standart]";?>" >
                    <table id='sample-table' class='table table-hover  tablesorter'>
                        <tr>
                            <td  width=100px;> 
                                <div style='float:left; width:70px;' >Daftar Isi </div>
                            </td>     
                            <td> 
                                <?PHP
                                echo "<select name='id_daftar_isi_ims' id='id_daftar_isi_ims'>";
                                $bag=mysqli_query($con,"select *from daftar_isi   where standart ='$h_cari[id_standart]' order by id_menu");
                                while($databag=mysqli_fetch_array($bag,MYSQLI_ASSOC)){
                                    echo"<option value=\"$databag[id_menu]\" >$databag[title]</option>";
                                }
                                echo"</select>";
                                ?>  
                            </td>
                        ...
                        <tr>
                            <td colspan=2>
                                <div id='paddingsaja' ><input type=submit class='btn btn-xs btn-warning' value=Simpan name='simpandata'></div>
                            </td>
                        </tr>
                    </table>
                    </form>
                ...
                <table class="table table-striped responsive-utilities jambo_table data">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <a <?php echo"onClick=\"return confirm ('Apakah Anda Yakin ingin Menghapus  Matrik ini ?')\"    href=?module=master&menu=m_matrik_ims&standart=$_GET[standart]&menu_isi=$_GET[id_menu]&id=$rs[id_matrik_ims]&act=hapus"; ?> class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Hapus </a>
                                                <a href="?module=master&menu=m_matrik_ims_detail&standart=<?php echo"$r[id_standart]";?>&daftar_isi=<?php echo"$rs[id_daftar_isi_ims]";?>"; class="btn btn-primary btn-xs"><i class="fa fa-edit"></i> Detail </a>
                                            </td>
                                        </tr>
                                        <?php
                                        $noa++;
