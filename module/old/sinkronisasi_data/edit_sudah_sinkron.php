  ...
  $show_nav=mysqlI_fetch_array(mysqlI_query($con,"select *from sinkronisasi_data where kks='$_GET[id]'"));
  ...
  <form onSubmit='return validasi(this)' method=POST >
    ...
    <tr>
      ...
      $c_nav=mysqli_fetch_array(mysqli_query($con,"select *from equipment_navitas where cause_code='$show_nav[cause_code]' "));
      echo"<option value=\"$c_nav[cause_code]\" > $c_nav[nama_system] | $c_nav[cause_code]</option>";
      $bid=mysqli_query($con,"select *from equipment_navitas where cause_code<>'$show_nav[cause_code]'   order by cause_code");
  ...
  if(isset($_POST[editdata_sinkron])){
    ...
    $simpan=mysqli_query($con,"UPDATE   sinkronisasi_data set cause_code='$_POST[cause_code]'
            where kks='$_POST[kks]'
     ");
    if($simpan>0){
      ?>
